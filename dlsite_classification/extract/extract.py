...
class ExtractFolder:
    ...
    async def make_tag(self, tags_table: dict[str, str]) -> Tag:
        ...
        async def _load(name, val):
            ...
            if eng_name in ["company", "title"]:
                ...
                temp[eng_name] = (int(data[0]), data[1] if len(data) == 2 else "")
                ...
                temp[eng_name] = (
                    [item.strip() for item in data if item.strip()] if data else None
                )
            else:
        ...
        await asyncio.gather(*[_load(name, val) for name, val in tags_table.items()])
        return Tag(**temp)
    ...
    def get_table(self, limit=100, offset=0) -> list:
        return list(
            islice(self.classification_table.items(), offset, offset + limit),
        )

    async def _scan(self, origin_folder_path):
        if not REGEX_COMPANY_FOLDER.match(origin_folder_path):
            return
        company_folder_path = os.path.join(self.path, origin_folder_path)
        ...
        self.classification_table[origin_folder_path] = Company(
            name=origin_folder_path, path=company_folder_path, work_item=work_item
        )
    ...
    async def scan_work(self, path: str, code: str) -> WorkInfo:
                ...
                try:
                    ...
                    if eng_name in ["company", "title"]:
                        minimal_data[eng_name] = (
                            {data[0]: data[1]} if len(data) > 1 else {data[0]: ""}
                        )
                    elif eng_name in ["introduction", "code", "request_failed"]:
                        minimal_data[eng_name] = "\n".join(data)
                    elif eng_name in ["star"]:
        ...
        work_info = WorkInfo(path=path, images=images, tag=tag)
        return work_info
